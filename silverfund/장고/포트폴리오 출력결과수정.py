...
class DB_Control:
    ...
    def Data_Update():
        ...
        DB_Control.DeleteAllFiles(file_path1)
        ...
        #데이터 통합과정    
        File_List = glob.glob(r'.\static\8분류\*.xls')
        ...
        for i in range(8,0,-1):
            ...
            result = ['위험등급']
            risk_data = pd.read_excel('./static/통합/통합.xls')
            ...
            data=data[1:]
            data.to_excel(f'./static/funds{i}.xlsx', index=False)
    
    ####10개의 결과화면 만들기
        for i in range(1,9):
            globals()[f"R_Data{i}"] = pd.read_excel(f'./static/funds{i}.xlsx').sort_values('이익1년').drop(['상품종류','설정원본','순자산','이익2년','이익4년'], axis=1)
    ###문제없이 작동
    ####퇴직연금 안정형
    ...
    def Data_Call():
        score = int(input())
        if score <=20 : 
            result = pd.read_excel('./static/result1.xlsx')
            result2 = pd.read_excel('./static/result6.xlsx')
            return result, result2
        elif score <=40 :
            result = pd.read_excel('./static/result2.xlsx')
            result2 = pd.read_excel('./static/result7.xlsx')
            return result, result2
        elif score <=60 :
            result = pd.read_excel('./static/result3.xlsx')
            result2 = pd.read_excel('./static/result8.xlsx')
            return result, result2
        elif score <=80 :
            result = pd.read_excel('./static/result4.xlsx')
            result2 = pd.read_excel('./static/result9.xlsx')
            return result, result2
        else:
            result = pd.read_excel('./static/result5.xlsx')
            result2 = pd.read_excel('./static/result10.xlsx')
            return result, result2
